...
### 1.3 États calculés dérivés (nommage explicite)

Pour éviter des conditions dispersées dans les composants, nommer explicitement dans le code ou la doc :

| Dérivé | Définition | Usage |
|--------|------------|--------|
| **isChromeVisible** | `state === 'expanded' \|\| state === 'compact'` | Header affiché (complet ou contracté). |
| **isChromeCollapsed** | `state === 'compact' \|\| state === 'hidden'` | Header non expanded (compact ou masqué). |

À exposer depuis le contexte ou un hook dérivé (ex. `useChromeDerivedState()`).

### 1.4 Fallback si `matchMedia` non disponible (hydratation)

- **Avant résolution client** : comportement **conservateur**.
- **Mode par défaut** tant que le device mode n’est pas confirmé : **non immersif** (tablette ou équivalent).
- Ne jamais déclencher le mode desktop immersif avant que `useInteractionMode()` ait retourné `'desktop'` côté client.

Objectif : aucun risque de desktop immersif (auto-hide) déclenché trop tôt au chargement ou en SSR.
...
- [x] Aucun flash visible au chargement.  
- [x] Aucune oscillation sur micro-scroll.  
- [x] Aucune réapparition parasite en lecture longue (grande card).  
- [x] Comportement identique sur au moins deux vues cockpit représentatives.
...
**DoD Phase 1B**  
- [x] Si frozen = true (test manuel ou via un overlay), le header ne se masque pas et ne se contracte pas.  
- [x] À la fermeture (frozen = false), le comportement reprend normalement.
...
**DoD Phase 2**  
- [x] iPad Pro (largeur type 1366 px) → mode **tablette**, jamais desktop immersif.  
- [x] Mobile → header **jamais** en hidden.  
- [x] Hover n’a **aucun effet** sur tactile (tablette/mobile).  
- [x] Bandeau réapparition : focus clavier OK, aria-label présent.
...
**DoD Phase 3**  
- [x] Ouverture d’un select ou du menu header → header ne se masque pas / ne se contracte pas.  
- [x] Activer “Garder le bandeau” → plus de masquage ni contraction automatique pour la session.
...
**DoD Phase 4**  
- [x] Sur iPhone (ou simulateur), pas de recouvrement encoche / barre d’accueil.  
- [x] Saisie dans un champ sur mobile : pas de saut du header/footer.  
- [x] Footer mobile : ligne compacte + ouverture drawer au tap.
...
**DoD Phase 5**  
- [x] Chaque révélation du header est associée à une cause (hover, scroll_up, focus, tap_trigger, manual_pin).  
- [x] Données exploitables pour ajuster timers et comportements.
...
## 8. Matrice de recette appareil (cible)

Matrice minimale pour éviter les oublis en validation. À cocher par phase / release.

| Appareil / navigateur | Comportement attendu | Phase cible |
|----------------------|----------------------|-------------|
| **Desktop souris** (Mac ou Win — Firefox ou Chrome) | Mode desktop immersif ; auto-hide après délai ; réapparition hover / scroll top / focus. | 1A, 2 |
| **iPad Pro / Safari** (largeur type 1366 px) | Mode **tablette** uniquement ; jamais desktop immersif ; pas de réapparition au hover. | 2 |
| **iPhone / Safari** | Sticky compact au scroll ; **safe areas** respectées ; pas de hidden ; footer compact + drawer au tap. | 4 |
| **Android / Chrome** (mobile) | Mobile compact ; pas de hidden ; comportement tactile cohérent. | 4 |
| **Desktop tactile** (si disponible) | Vérifier **absence d’effet hover parasite** ; doit rester en mode tablette/consultation. | 2 |

---

**Version** : 1.1  
**Répertoire** : ZeDocs/web53  
**Statut** : **Implémentation terminée** (mars 2026). Phases 0 à 5 livrées et déployées (laplatine2026, o19). Décisions verrouillées, DoD par phase cochés, frozen et scrollRoot clarifiés ; états dérivés nommés, fallback matchMedia, matrice recette.  
**Suite** : Backlog → `BACKLOG_SCRUM_CHROME_ADAPTATIF_v1.0.md` ; rapport détaillé → `RAPPORT_IMPLEMENTATION_CHROME_ADAPTATIF_v1.0.md`.
