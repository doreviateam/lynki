...
Les vues **métier** de l’app HelloAsso n’affichent pas le vocabulaire brut du connecteur (`slug`, `Form type`, `Event`, `Sync commandes`, etc.) : libellés de champs, filtres, boutons et textes d’aide sont formulés en **langage produit**. **Type** affiche une traduction métier des valeurs API courantes (ex. **Événement**). **Organisation** peut afficher un **nom lisible** si le champ facultatif est renseigné dans Paramètres HelloAsso (Members) ; sinon la référence technique. Filtre liste billetteries : **Billetteries** (événements). Champs techniques réservés aux administrateurs sur la fiche si besoin.
