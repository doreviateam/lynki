...
* **Preuve de version déployée :** en pied de page Linky, la ligne **UI** + **hash court git** doit correspondre au **commit** attendu (`NEXT_PUBLIC_LINKY_UI_BUILD_REF` injecté au build Docker). Si l’UI ou les libellés (ex. W60-005, W60-101) ne matchent pas le dépôt alors qu’un déploiement vient d’être fait : vérifier que Caddy pointe bien vers **`linky_generic`** pour `lab.linky` (voir `units/gateway/Caddyfile`) et que ce conteneur a bien été reconstruit (écart typique : seul `linky_lab_laplatine2026` avait été rebuildé).  
