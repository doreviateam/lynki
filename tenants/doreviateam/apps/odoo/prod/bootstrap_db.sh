...
# Supprimer \r du .env pour que compose et source utilisent la même valeur
sed -i 's/\r$//' .env
...
$COMPOSE down
docker volume rm odoo_prod_o19_db 2>/dev/null || true
...
PY
    ...
    exec odoo -c /tmp/odoo.conf -d doreviateam -i base --load-language=fr_FR --stop-after-init
  '
...
echo "[bootstrap] URL : https://odoo.doreviateam.com/web/login"
