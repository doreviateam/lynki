#!/bin/bash
# Script de déploiement Sprint 8 - Options A + B (prev_hash)
# Version: 1.6.1

set -e

echo "🚀 Déploiement Sprint 8 - Options A + B (prev_hash)"
echo "==================================================="
echo ""
...
# 4. Note sur les migrations
echo ""
echo "📊 Note: Aucune migration DB requise"
echo "   Les améliorations utilisent les structures existantes"
...
if command -v curl &> /dev/null; then
    ...
    HEALTH=$(curl -s http://localhost:8080/health 2>&1 || echo "")
    if echo "$HEALTH" | grep -q "ok\|200"; then
...
echo ""
echo "=========================================="
echo -e "${GREEN}✅ Déploiement Sprint 8 (Options A + B) terminé${NC}"
echo "=========================================="
echo ""
echo "📋 Améliorations déployées:"
echo "   ✅ Option A: Endpoint ledger/export avec paramètre document_id"
echo "   ✅ Option B: Champ prev_hash dans endpoints proof"
echo ""
echo "📚 Documentation:"
echo "   - docs/PROOF_API.md (mise à jour)"
echo "   - docs/LEDGER_EXPORT_API.md (nouveau)"
echo "   - docs/INFORMATION_EQUIPE_ODOO_OPTIONS_AB_DEPLOYEES.md"
echo ""
echo "🧪 Tests:"
echo "   curl -X GET http://localhost:8080/api/v1/proof/account_move/123 \\"
echo "     -H 'Authorization: Bearer YOUR_TOKEN'"
echo "   # Vérifier que prev_hash est présent dans la réponse"
echo ""
echo "   curl -X GET 'http://localhost:8080/api/v1/ledger/export?document_id=<uuid>&format=json' \\"
echo "     -H 'Authorization: Bearer YOUR_TOKEN'"
echo "   # Vérifier le nouveau format de réponse"
echo ""
