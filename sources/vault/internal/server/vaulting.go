...
func RegisterVaultingRoutes(app *fiber.App, db *storage.DB, cfg *config.Config, log *zerolog.Logger) {
	...
	// POST /api/v1/account-move-lines — Ingestion écritures comptables tous journaux (Sprint 04 T21)
	app.Post("/api/v1/account-move-lines", handlers.AccountMoveLinesIngestHandler(db))

	log.Info().Msg("Vaulting routes enabled: /api/v1/invoices, /api/v1/events, /api/v1/payments, /api/v1/payroll, /api/v1/payroll-od-lines, /api/v1/account-move-lines, /api/v1/proof/*, /api/v1/expected-counts")
}
