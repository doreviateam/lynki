...
// ArByPartnerAggregationHandler gère GET /ui/aggregations/ar-by-partner (SPEC AR by Partner v1.0.3)
// Paramètres : tenant (requis), date_debut, date_fin, as_of_date, company_id (opt), overdue (bool), limit (opt, défaut 50)
func ArByPartnerAggregationHandler(db *storage.DB) fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		// Alias compat date_from / date_to
		if dateDebut == "" {
		...
		ctx := c.Context()
		resp, err := db.ArByPartnerAggregation(ctx, tenant, dateDebut, dateFin, asOfDate, companyID, overdueOnly, limit)
		if err != nil {
