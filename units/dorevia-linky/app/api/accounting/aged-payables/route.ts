...
export interface LynkiAgedPayablesResponse {
  restitution_id: "lynki.accounting.aged_payables";
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const vaultRes = await fetch(
      `${VAULT_URL}/api/accounting/aged-payables?${params}`,
    ...
    if (vaultRes.ok) {
      ...
      const response: LynkiAgedPayablesResponse = {
        restitution_id: "lynki.accounting.aged_payables",
        ...
        aging_basis: typeof raw.aging_basis === "string" ? raw.aging_basis as LynkiAgedPayablesResponse["aging_basis"] : undefined,
    ...
    if (strict) {
      return NextResponse.json({ error: "vault_aged_payables_unavailable", vault_status: vaultRes.status }, { status: 502 });
    }
  } catch {
    if (strict) {
      return NextResponse.json({ error: "vault_aged_payables_unreachable" }, { status: 502 });
    }
  }

  const stub: LynkiAgedPayablesResponse = {
    restitution_id: "lynki.accounting.aged_payables",
