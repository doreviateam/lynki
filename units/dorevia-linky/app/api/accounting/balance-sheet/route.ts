...
/** Ligne agrégée par classe PCG (Sprint 07 T39) */
...
/** Réponse lynki.accounting.balance_sheet */
export interface LynkiBalanceSheetResponse {
...
function jsonBalanceSheet(body: LynkiBalanceSheetResponse, status = 200): NextResponse {
  const res = NextResponse.json(body, { status });
  res.headers.set("X-Lynki-Accounting-Source", body.data_source);
  return res;
}

// GET /api/accounting/balance-sheet — proxy Vault Sprint 07 T39
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const vaultRes = await fetch(
      `${VAULT_URL}/api/accounting/balance-sheet?${params}`,
    ...
    if (vaultRes.ok) {
      ...
      const ref =
        typeof raw.referentiel_version === "string" && raw.referentiel_version.length > 0
          ...
          : referentielFallback;
      const response: LynkiBalanceSheetResponse = {
        restitution_id: "lynki.accounting.balance_sheet",
        ...
        perimeter_note:
          typeof raw.perimeter_note === "string"
            ? raw.perimeter_note
            : "Premier incrément — agrégation par classe PCG (classes 1–5).",
      ...
      return jsonBalanceSheet(response);
    }

    if (strictAccounting) {
      return NextResponse.json(
        {
          error: "vault_balance_sheet_unavailable",
    ...
    if (strictAccounting) {
      return NextResponse.json({ error: "vault_balance_sheet_unreachable" }, { status: 502 });
    }
  }

  const stub: LynkiBalanceSheetResponse = {
    restitution_id: "lynki.accounting.balance_sheet",
