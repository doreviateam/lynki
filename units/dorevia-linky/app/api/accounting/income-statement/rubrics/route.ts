...
/** Ligne de rubrique comptable Lynki (Sprint 08 T44) */
export interface RubricLine {
  rubric_id: string;
  label: string;
  amount: number;
  section: string; // "exploitation" | "resultats"
  account_filter?: string[]; // préfixes de comptes pour drill BG (Sprint 09 T49)
}

/** Réponse lynki.accounting.income_statement detail_level=rubrics */
export interface LynkiIncomeStatementRubricsResponse {
...
function jsonResponse(body: LynkiIncomeStatementRubricsResponse, status = 200): NextResponse {
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const vaultRes = await fetch(
      `${VAULT_URL}/api/accounting/income-statement/rubrics?${params}`,
    ...
    if (vaultRes.ok) {
      ...
      const response: LynkiIncomeStatementRubricsResponse = {
        restitution_id: "lynki.accounting.income_statement",
    ...
    if (strict) {
      return NextResponse.json({ error: "vault_income_statement_rubrics_unavailable", vault_status: vaultRes.status }, { status: 502 });
    }
  } catch {
    if (strict) {
      return NextResponse.json({ error: "vault_income_statement_rubrics_unreachable" }, { status: 502 });
    }
  }

  const stub: LynkiIncomeStatementRubricsResponse = {
    restitution_id: "lynki.accounting.income_statement",
