...
/**
 * GET /api/bfr-evolution — proxy vers Vault GET /ui/aggregations/bfr-series (ADR-0010, E4).
 * Retourne { series: { period, amount }[] } pour le bloc Évolution BFR (bfr_net = AR − AP).
 */
export async function GET(request: NextRequest) {
  ...
  try {
    const res = await fetch(`${vaultBase}/ui/aggregations/bfr-series?${qs}`, {
