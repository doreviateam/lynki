...
interface SalesCardProps {
  data: SalesAggregation | null;
  error?: string;
  loading?: boolean;
  /** Nombre de factures de vente postées dans Odoo (dénominateur du ratio Certifié). */
...
export function SalesCard({
  ...
  if (loading) {
    return (
      <section className={cardStyle}>
        <div className={`${cardTitle} mb-2.5`}>Ventes</div>
  ...
  // Ratio Certifié = toujours global. "X factures concernées" = data.invoices_count (période).
  ...
  const certifiedLabel =
    ...
    postedSalesCount > 0
      ...
      : postedSalesCount != null &&
          (typeof globalVaulted !== "number" || postedSalesCount <= 0)
        ? `Certifié : ${globalVaulted ?? 0} / ${postedSalesCount}`
        : data.verifiable
          ? "Données certifiées"
          : null;
