...
export const COCKPIT_BUSINESS_UNAVAILABLE_BODY =
  "Les indicateurs d’activité commerciale n’ont pas pu être chargés (réseau ou réponse vide). Réessayez ou vérifiez la configuration du tenant.";

export function CockpitBusinessLoadingSkeleton({
  label,
  id,
  rows = 4,
}: {
...
export function CockpitBusinessUnavailable({
  title,
  onRetry,
  attemptHint,
  id,
  ...
  return (
    ...
    >
      ...
      <p className="mt-1 text-xs leading-relaxed text-[var(--text-secondary)]">{COCKPIT_BUSINESS_UNAVAILABLE_BODY}</p>
...
export function CockpitBusinessEmptyNotice({ children }: { children: ReactNode }) {
  return (
...
export function CockpitBusinessPartialBanner({ children }: { children: ReactNode }) {
  return (
