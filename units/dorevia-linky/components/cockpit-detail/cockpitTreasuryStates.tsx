...
/** Message réseau / agrégat indisponible — ton cockpit trésorerie (pas bilan de trésorerie normé). */
export const COCKPIT_TREASURY_UNAVAILABLE_BODY =
  "Les indicateurs de trésorerie n'ont pas pu être chargés (réseau ou réponse vide). Réessayez ou vérifiez la configuration du tenant.";

export function CockpitTreasuryLoadingSkeleton({
  label,
  id,
  rows = 4,
}: {
...
export function CockpitTreasuryUnavailable({
  title,
  onRetry,
  attemptHint,
  id,
  ...
  return (
    ...
    >
      ...
      <p className="mt-1 text-xs leading-relaxed text-[var(--text-secondary)]">{COCKPIT_TREASURY_UNAVAILABLE_BODY}</p>
...
export function CockpitTreasuryEmptyNotice({ children }: { children: ReactNode }) {
  return (
...
/** KPI présent mais détail rapprochement / soldes absent — pas d'invention de chiffres. */
export function CockpitTreasuryPartialBanner({ children }: { children: ReactNode }) {
  return (
