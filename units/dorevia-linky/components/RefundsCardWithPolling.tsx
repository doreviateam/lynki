...
interface RefundsCardWithPollingProps {
...
export function RefundsCardWithPolling({
  initialClientData,
  initialSupplierData,
  initialErrorClient,
    ...
    const fetchBoth = async () => {
      ...
      try {
        const [resClient, resSupplier] = await Promise.all([
          fetch(`/api/adjustments?${new URLSearchParams({ ...baseParams, event_type: "refund.customer.paid" }).toString()}`, {
            cache: "no-store",
            headers: { Accept: "application/json" },
          }),
          fetch(`/api/adjustments?${new URLSearchParams({ ...baseParams, event_type: "refund.supplier.received" }).toString()}`, {
            cache: "no-store",
            headers: { Accept: "application/json" },
          }),
        ]);
